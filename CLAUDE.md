...
- `backend/agent/fixed_pipeline/` — **Fixed Pipeline design**: structured routing + deterministic pipelines with micro-agents. `FixedPipelineAgent` classifies user intent via a Routing Agent, then dispatches to pipeline classes (find_jobs, research_url, track_crud, query_jobs, todo_mgmt, profile_mgmt, prepare, compare, research, general, multi_step). Each pipeline uses scoped LLM calls (micro-agents) only where reasoning is needed. Reuses `DefaultOnboardingAgent` and `DefaultResumeParser`. Key files: `agent.py` (main entry), `routing.py` (intent classification), `schemas.py` (Pydantic schemas), `context.py` (per-request cache), `streaming.py` (SSE helpers), `prompts.py` (all prompts), `micro_agents.py` (BaseMicroAgent + implementations), `entity_resolution.py` (job ref → ID with tiered scoring), `pipeline_base.py` (`Pipeline` base class + `ToolResult` container), `pipelines/` (one file per pipeline type, each a `Pipeline` subclass with a `run()` wrapper).
...
- `AGENT_DESIGN` — agent design/strategy to use (default: `default`); options: `default` (monolithic ReAct), `fixed_pipeline` (routing + micro-agents); corresponds to a sub-package in `backend/agent/`
