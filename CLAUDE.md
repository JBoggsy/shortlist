...
- `backend/app.py` — Flask app factory (`create_app`); initializes Flask-Migrate and applies database migrations on startup via `_apply_migrations()` (handles fresh, pre-migration, normal, and corrupted databases)
...
- `backend/llm/model_listing.py` — `list_models()` functions for each provider (uses raw SDKs to query available models); `MODEL_LISTERS` registry; `pick_best_ollama_model()` ranks installed Ollama models against `_PREFERRED_OLLAMA_MODELS` list (llama3.3, llama3.1, qwen3.5, qwen3, qwen2.5, gemma3, ...), then by parameter size, then alphabetically — used by the providers endpoint and setup wizard for smart default model selection
...
- `frontend/src/components/SetupWizard.jsx` — First-time setup wizard (centered modal, 5 steps: welcome → provider selection → API key entry with inline how-to guide + test connection → integration keys (Tavily + RapidAPI) with inline how-to guides → done); auto-opens for new users instead of Settings page; adds elapsed/timeout feedback during connection tests, eagerly detects installed Ollama models for the override field, and shows a stronger scroll cue on the integrations step; for Ollama, fetches `fetchProviders()` to use the ranked `default_model` (from `pick_best_ollama_model()`) instead of first alphabetical model; calls `onComplete()` to launch onboarding chat or `onClose()` to dismiss
