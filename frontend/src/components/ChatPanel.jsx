                  ...
                  <div key={i}>
                    ...
                    {msg.role === "assistant" && msg.id && !isStreaming && (
                      <div className="flex gap-1 mt-1 ml-1">
                        <button
                          onClick={() => {
                            sendMessageFeedback(currentConversation.id, msg.id, "thumbs_up").catch(() => {});
                            setMessageFeedback((prev) => ({ ...prev, [msg.id]: "thumbs_up" }));
                          }}
                          className={`p-1 rounded text-xs transition-colors ${
                            messageFeedback[msg.id] === "thumbs_up"
                              ? "text-green-600"
                        ...
                        <button
                          onClick={() => {
                            sendMessageFeedback(currentConversation.id, msg.id, "thumbs_down").catch(() => {});
                            setMessageFeedback((prev) => ({ ...prev, [msg.id]: "thumbs_down" }));
                          }}
                          className={`p-1 rounded text-xs transition-colors ${
                            messageFeedback[msg.id] === "thumbs_down"
                              ? "text-red-500"
                              : "text-gray-300 hover:text-gray-500"
                          }`}
                          title="Not helpful"
